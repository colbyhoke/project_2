...
var staDisplay = document.getElementById('state');
var couDisplay = document.getElementById('county');

// Mapbox access. We used Mapbox GL to "construct" the map in order to join data on a a county level variable (fips).
...
map.on('load', () => {d3.json("/api/v1.0/masks").then((data) => {initFeatureState(data)});
...
const initFeatureState = (data) => {
  map.addSource('albersusa', {
    type: 'vector',
    url: 'mapbox://lobenichou.albersusa', // URL for the Albers Tileset provided by the Mapbox Developer of the Style.
...
map.on('mousemove', 'albersusa-fill', (e) => {
  ...
  const description = 
  `<p>${d3.format("")((e.features[0].state.mask_always)*100)}% of the people in ${e.features[0].properties.county_name}, ${e.features[0].properties.state_abbrev} always wear a mask</p>`;
  ...
  popup // Determines where, what, and how the pop up "reacts".
    ...
    staDisplay.innerHTML = e.features[0].properties.state_name
    couDisplay.innerHTML = e.features[0].properties.county_name
});
